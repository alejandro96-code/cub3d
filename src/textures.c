...
// Error de textura con limpieza
static void	texture_error(const char *msg, t_mlx *mlx, t_g *g)
{
	destroy_window(mlx);
	free_g(g);
	error_exit(msg);
...
static void	load_one(char *path, t_mlx *mlx, int idx, t_g *g)
{
	...
	if (!g->tex_img_ptr[idx])
		texture_error(ERROR_TEXTURE_LOAD, mlx, g);
	g->tex_buffer[idx] = (int *)mlx_get_data_addr(g->tex_img_ptr[idx],
			&bpp, &line_len, &endian);
	if (!g->tex_buffer[idx])
		texture_error(ERROR_TEXTURE_BUFFER, mlx, g);
}
